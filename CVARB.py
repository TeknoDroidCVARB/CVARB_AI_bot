...
pTELEGRAM_TOKEN = '6649599069:AAFeoVSNrzOxfRs1ZgLU1rbLpEqJElZNkSo'
...
┃➥ ➕ /headers - İstediğin Sitenin Headers'ini Çek.'
┃➥ 🌥️ /hava_durumu - İstediğin Bölgenin Hava Durumunu Öğren.
┃➥ 🐚 /tiktok_info - İstediğin Tiktok Hesabından Bilgi Al.'
┃➥ 〽️ /tel_info - İstediğin Telefondan Bilgi Al.
┃➥ 🍄 /insta_info - İstediğin İnsta Hesabından Bilgi Al.
╰━━━━━━━━━━━━━━━━━━━━━━━
...
┃➥ 🃏 /cevrimici_oyun: - Çok Daha Fazla Ve Çevrimiçi Oyun.
...
┃➥ 📨 /fake_email - Kendine Bedava Tek Kullanımlık Mail Al
╰━━━━━━━━━━━━━━━━━━━━━━━"""
...
def headers_command(message):
    ...
    if len(args) < 2:
        bot.reply_to(message, "Bu Komut İle Siteden Headers Alabilirsin. Örnegin: \n/headers https://example.com")
    else:
        site = args[1].replace("https://", "").replace("http://", "").split("/")[0]  # Protokol ve yolu kaldır
        headers = get_headers(args[1])
        file_name = f"{site} Headers.txt"  # Dosya adını belirle
...
# Instaloader sınıfından bir nesne oluştur
...
def get_insta_info(username):
    ...
    try:
        ...
        info = f"Kullanıcı adı: {profile.username}\n\n"
        info += f"Tam adı: {profile.full_name}\n\n"
        info += f"Profil fotoğrafı URL: {profile.profile_pic_url}\n\n"
        info += f"Takipçi sayısı: {profile.followers}\n\n"
        info += f"Takip ettiği sayısı: {profile.followees}\n\n"
        info += f"Bio: {profile.biography}\n\n"
        info += f"Web sitesi: {profile.external_url}\n\n"
        info += f"Paylaştığı gönderi sayısı: {profile.mediacount}\n\n"
        return info
...
def send_insta_info(message):
    args = message.text.split()
    if len(args) < 2:
        bot.reply_to(message, "Kullanım: /insta_info <instagram_kullanıcı_adı>\nÖrnek: /insta_info teknodroidevreni")
    else:
        username = args[1].lstrip('@')  # Kullanıcı adının başındaki @ işaretini kaldır
        info = get_insta_info(username)
        bot.reply_to(message, info)
        
@bot.message_handler(commands=["cevrimici_oyun"])
@safe_execute
@check_membership
@user_is_logged_in
def oyun_sec(message):
    markup = telebot.types.InlineKeyboardMarkup()
    oyunlar = [
        ("🧠 Zeka Oyunu", 'iq'),
        ("🏃‍♂️ Parkur Oyunu", 'parkour'),
        ("⚽ Penaltı Atışı Oyunu", 'penalty'),
        ("🦶 Serbest Vuruş Oyunu", 'kick'),
        ("❌⭕ XO Oyunu", 'xo'),
        ("🚇 Metro Oyunu", 'subway'),
        ("🏜 Piramitler Oyunu", 'temple'),
        ("🚀 JetPack Oyunu", 'jetpack'),
        ("🎨 Çizim Oyunu (Çevrimiçi)", 'draw'),
        ("🤔 Google Feud Oyunu", 'google'),
        ("⚔️ Krunker Oyunu (Savaş)", 'war'),
        ("🐍 Canlı Oyun", 'slither'),
        ("😺 PopCat Oyunu", 'cat')
    ]
    for oyun in oyunlar:
        markup.add(telebot.types.InlineKeyboardButton(text=oyun[0], callback_data=oyun[1]))
    
    bot.send_message(message.chat.id, "🎮 Oynamak istediğiniz oyunu seç:", reply_markup=markup)

@bot.callback_query_handler(func=lambda call: True)
def oyun_baslat(call):
    oyunlar = {
        'draw': ('Çizim Oyunu', 'https://gartic.io/'),
        'google': ('Google Feud Oyunu', 'https://googlefeud.com/'),
        'war': ('Krunker Oyunu', 'https://krunker.io/'),
        'slither': ('Canlı Oyun', 'https://slither.io/'),
        'cat': ('PopCat Oyunu', 'https://popcat.click/'),
        'iq': ('Zeka Oyunu', 'https://poki.com/ar/g/brain-test-tricky-puzzles#fullscreen'),
        'parkour': ('Parkur Oyunu', 'https://poki.com/ar/g/roper#fullscreen'),
        'penalty': ('Penaltı Atışı Oyunu', 'https://poki.com/ar/g/penalty-shooters#fullscreen'),
        'kick': ('Serbest Vuruş Oyunu', 'https://poki.com/ar/g/free-kick-shooter'),
        'xo': ('XO Oyunu', 'https://poki.com/ar/g/tictactoe#fullscreen'),
        'subway': ('Metro Oyunu', 'https://poki.com/ar/g/subway-surfers#fullscreen'),
        'temple': ('Piramitler Oyunu', 'https://poki.com/ar/g/temple-run-2#fullscreen'),
        'jetpack': ('JetPack Oyunu', 'https://poki.com/ar/g/jetpack-joyride#fullscreen')
    }

    oyun_adi, oyun_url = oyunlar.get(call.data, ('Bilinmeyen', '#'))
    markup = telebot.types.InlineKeyboardMarkup()
    markup.add(telebot.types.InlineKeyboardButton(text=f"🎮 {oyun_adi} Oyna!", web_app=telebot.types.WebAppInfo(oyun_url)))
    bot.reply_to(call.message, f"🚀 Oyuna başlamak için aşağıdaki butona tıkla!", reply_markup=markup)

# Kullanımı anlatan mesajı gönderir
...
def send_tel_info_welcome(message):
    ...
    if len(msg_parts) == 1:
        # Kullanıcı yalnızca /tel_info komutunu girdiğinde kullanımı anlat
        bot.reply_to(message, "Numara bilgisi için lütfen telefon numaranızı /tel_info komutu ile birlikte gönderin. Örneğin:\n/tel_info +905555555555")
    else:
        # /tel_info ile birlikte numara da girildiyse numara bilgisini işle
        get_phone_info(message)
...
def get_phone_info(message):
    try:
        ...
        response = f"Numara: {phone_number}\n\n" \
                   f"Zaman Dilimi: {time_zone}\n\n" \
                   f"Operatör: {operator}\n\n" \
                   f"Bölge: {region}"
    except Exception as e:
        response = "Bir hata oluştu, lütfen geçerli bir telefon numarası formatı kullanın."
...
def send_tiktok_info(message):
    ...
    if len(msg_parts) == 1:
        bot.reply_to(message, "Bu Komut İle Tiktok Kullanıcı Adından Bilgi Tollayabilirsin. Örneğin: /tiktok_info @kullaniciadi")
    else:
        kullanıcı = msg_parts[1]
        if kullanıcı.startswith('@'):
            kullanıcı = kullanıcı[1:]  # '@' karakterini kaldır
        tiktok_info = insta_info_cikar(kullanıcı)
        bot.reply_to(message, tiktok_info, parse_mode='HTML')

def insta_info_cikar(kullanıcı):
    ...
    data = {
        'unique_id': kullanıcı,
        'purchaseTELEGRAM_TOKENs': []
    }
    try:
        cevap = requests.post(url, headers=headers, json=data).json()
        user_data = cevap['data']['user']['user']
        stats = cevap['data']['user']['stats']
        ...
        sonuc = f'''
<b>&lt; &lt; &lt;  𝙄𝙎𝙏𝙀 𝙂𝙄𝙍𝘿𝙄𝙂𝙄𝙉 𝙃𝙀𝙎𝘼𝘽𝙄𝙉 𝘽𝙄𝙇𝙂𝙄𝙇𝙀𝙍𝙄 &gt; &gt; &gt;</b>


<b>IՏIᗰ:</b> {name}

<b>KᑌᒪᒪᗩᑎIᑕI ᗩᗪI:</b> {kullanıcı}

<b>TᗩKIᑭᑕI:</b> {followers}
...
<b>ᘜIᘔᒪIᒪIK ᗪᑌᖇᑌᗰᑌ:</b> {'Hesap Gizli' if priv else 'Hesap Gizli Değil'}
...
'''
        ...
        return "Bir hata oluştu, lütfen geçerli bir TikTok kullanıcı adı gir."
    
def check_inbox_and_notify(user_id, emaill):
    while True:
        messages = Email(emaill["session"]).inbox()
        if messages:
            bot.send_message(user_id, f"{messages} GELEN MESAJ")
            break  # Mesaj geldiğinde döngüyü sonlandır
...
@user_is_logged_in
@user_is_premium
def generate_and_monitor_email(message):
    emaill = Email().Mail()
    email = emaill["mail"]
    bot.send_message(message.chat.id, f"E-posta'nız: {email}\n\nE Mail'e Kod Gönderildiğinde Sana Mesajı İleticem.")
    
    # Arka planda inbox kontrolü için bir thread başlat
    threading.Thread(target=check_inbox_and_notify, args=(message.chat.id, emaill)).start()
